from game import ALLOWED_SYMBOLS, Brick
...
def display_field(field, active_brick=None):
    ...
    for r in range(field.height):
        ...
        for c in range(field.width):
            ...
            if (
                active_brick
                and active_brick.row
                <= r
                < active_brick.row + (3 if active_brick.orientation == "V" else 1)
                and (
                    (
                        active_brick.orientation == "H"
                        and active_brick.col <= c < active_brick.col + 3
                    )
                    or (active_brick.orientation == "V" and c == active_brick.col)
                )
            ):
                ...
                cell = (
                    active_brick.symbols[idx] if cell is None else cell
                )  # show brick symbol if empty, but if occupied, show grid?
